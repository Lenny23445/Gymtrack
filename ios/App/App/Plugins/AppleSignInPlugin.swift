...
extension AppleSignInPlugin: ASAuthorizationControllerDelegate {
    public func authorizationController(controller: ASAuthorizationController,
        ...
        guard let credential = authorization.credential as? ASAuthorizationAppleIDCredential,
            ...
            pendingCall?.reject("Credential konnte nicht gelesen werden")
            pendingCall = nil
            return
        }
        pendingCall?.resolve([
    ...
    public func authorizationController(controller: ASAuthorizationController,
        ...
        // Code 1001 = Nutzer hat abgebrochen — kein Fehler
        if nsErr.domain == ASAuthorizationError.errorDomain && nsErr.code == 1001 {
            pendingCall?.reject("cancelled")
        } else {
            pendingCall?.reject("Sign in with Apple fehlgeschlagen: \(error.localizedDescription)")
        }
        pendingCall = nil
    }
}
