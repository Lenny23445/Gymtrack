...
public class SpotlightPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    @objc func indexExercises(_ call: CAPPluginCall) {
        ...
        let items: [CSSearchableItem] = rawItems.compactMap { item in
            ...
            attrs.title              = name
            attrs.contentDescription = "\(muscle) · GymTrack Übung"
            attrs.keywords           = [name, muscle, "gymtrack", "übung", "training", "fitness"]
            let si = CSSearchableItem(
                uniqueIdentifier: "gymtrack.exercise.\(id)",
        ...
        CSSearchableIndex.default().indexSearchableItems(items) { error in
            if let error = error {
                call.reject("Indizierung fehlgeschlagen: \(error.localizedDescription)")
            } else {
                call.resolve(["indexed": items.count])
            }
        }
    }
    ...
    @objc func clearIndex(_ call: CAPPluginCall) {
        CSSearchableIndex.default().deleteSearchableItems(
            ...
            if let error = error {
                call.reject("Index löschen fehlgeschlagen: \(error.localizedDescription)")
            } else {
                call.resolve()
            }
        }
