...
| `js/app-community.js` | 2.961 | Feed-Pager, Share-Card, Posts |
| `js/app-crew.js` | 640 | Crews (gemeinsames Wochenziel) |
...
**Einstellungen = Quadranten + kleine Liste (Umbau 07.08.2026).** Statt fünfzehn Abschnitten untereinander: Premium-Karte, dann `.set-grid` mit vier `.set-tile`-Kacheln (Training · Community · Aussehen · Konto), die je ein Sheet öffnen (`ov-set-training`, `ov-set-community`, `ov-set-look`, `ov-set-account`), darunter die Liste „Weiteres" (Erinnerungen, Apple Health, Progression, Feedback, Datenschutz, Admin) und die Versionszeile `.set-foot` — die ist zugleich der Einstieg in den Admin-Modus (5× tippen → `adminTapCount()`; die Zeile mit `id="admin-section"` war seit dem Modul-Split verwaist, `updateAdminUI()` findet sie jetzt wieder). Die Kachel-Untertitel zeigen den aktuellen Stand und werden von `_renderSettingsTiles()` (in `js/app-workout.js`, gerufen aus `renderSettings()`) gefüllt. **Alle IDs der Regler sind unverändert** — die Karten wurden nur in die Sheets verschoben, kein Handler musste angefasst werden. Symbole sind Inline-SVG im Header-Stil (`.set-ico`, `.ico.set-ico-row`), keine Emojis mehr.

**KI-Bubble folgt der Tableiste.** `_aibSyncVisibility()` (in `js/app-coach-setup.js`) schaltet `body.ai-hidden` jetzt nach einer festen Regel: sichtbar genau dann, wenn die Tabbar sichtbar ist (`_aibTabbarSichtbar()` prüft `display`/`visibility`/`opacity` **und** ob ein offenes `.ov` mit höherem z-index sie überdeckt — Level-Vollbild `ov-lvlplate` liegt bei 1004, die Bubble bei 1060 und schwebte dort früher allein über dem Bild). Einzige Ausnahme: laufendes Training (`isWorkoutActive()` / `body.wk-active` / offenes `ov-wk`) — da bleibt der Coach durchgehend erreichbar. Sheets (z 900) liegen unter der Leiste, dort bleibt beides sichtbar. `updateWkMiniVisibility()` ruft die Synchronisierung mit auf, weil sich genau dort der Trainings-Zustand ändert.
...
S = {
  ...
  crewId,                                      // Crew-Zuordnung (Doc-ID = Beitrittscode), genau eine Crew
  gymName, gymLat, gymLng                      // eigenes Gym für die Karte
}
```

## Features
Übungen + Muskelgruppen-Filter · Training starten/loggen, Gewichtsvorschläge · 1RM (Epley) + Chart · Statistik Modus-Switcher (Muskeln/PPL/Ober-Unter) · Bottom Sheets Swipe-to-dismiss · 4 Themes · Dackel-Begleiter · Auto-Update (sw.js-Direktvergleich) · Changelog-Popup · Cardio-Timer + SW-Notification · Cloud-Sync Firebase · Aktives Training überlebt App-Neustart (localStorage `gt_active_wk`, Restore via `_restoreActiveWk()` im INIT, 8h-TTL) · Supersätze (`log.ssGroup`, Pause erst wenn alle Partner Satz N fertig) · Plate Calculator im Gewichts-Wheel (`_renderPlateCalc`, Stange via `S.plateBar`, nur lokal) · Herzfrequenz im Training (HealthKitPlugin `getLatestHeartRate`, JS-Polling 15 s via `_startHrPolling`, nur nativ) · Onboarding für neue User (`maybeStartOnboarding` im INIT, Fullscreen `#ob-screen`, JS-gerendert via `renderOb`, Plan-Empfehlung aus `PLAN_TEMPLATES` via `_applyTemplateCore`, Flag `S.onboarded`) · Community = eigener 5. Tab (`#pg-freunde`, `renderFriendsTab`). **Zwei Zonen, keine Chip-Leiste mehr (Umbau 07.08.2026).** Oben nur noch `#soc-zone-toggle`: **Community** (öffentlicher Feed, `_renderFeed`) und **Freunde**. Die alte Chip-Reihe `#fr-seg` (friends/feed/crew/board/map) ist **entfernt** — fünf Ansichten hinter fünf Klicks, keine beim Öffnen sichtbar. Die Freunde-Zone ist jetzt EINE scrollende Seite `_renderFrHome` (`_socTab='home'`): eigene Level-Karte → Anfragen → Crew-Kurzblock (`_crewHomeBlock`, Balken + Streak, Tipp öffnet die volle Crew) → Freundesliste (erste 4, `_frLimit`) → Top-3-Rangliste (`_renderFrBoardMini`) → Karten-Einstieg (`_renderFrMapMini`, bewusst **keine** zweite Leaflet-Instanz — die grosse Karte misst sich an der Bildschirmhöhe). Die vollen Ansichten (`_socTab` = `friends`/`crew`/`board`/`map`) hängen an „alle anzeigen" und tragen oben `_socBackBar()` → `setSocTab('home')`; ohne die wäre man in Rangliste/Karte gefangen. **Es gibt nur noch EINEN Feed — den öffentlichen (Umbau 07.08.2026, zweite Stufe).** Der Umschalter `.cpg-seg` („Alle · Nur Freunde"), `setCpgMode` und die Variable `_cpgMode` sind **entfernt**; `_cpgLoad(force)` kennt nur noch den collectionGroup-Zweig auf `visibility=='public'` (Index nötig). Auch der Share-Flow hat nur noch **ein** Ziel: `_shfShareStep` zeigt allein die Community-Zeile, `_shfPublish` schreibt genau ein Doc mit `visibility:'public'` — ein `'friends'`-Post entsteht nirgends mehr (die Firestore-Rule erlaubt den Wert weiter, damit alte Clients nichts brechen). Alt-Posts mit `visibility:'friends'` werden nicht mehr gelesen und laufen über die 7-Tage-TTL aus. Push-Regel dazu: der Freundes-Listener (`_friendPostNotifStart`) feuert jetzt unabhängig vom `visibility`-Wert, sobald ein **Freund** postet, und der Community-Listener (`_communitySub`) überspringt dafür alle UIDs aus `S.friends` — sonst zählte dieselbe Nachricht zweimal. **Share-Card:** 6 Layouts (`SHARE_LAYOUTS`: classic/bold/clean/minimal/frame/stats) + Farbpaletten (`SHF_PALETTES`, Index 0 = App-Theme, Swatch-Reihe `#shf-pal` im Editor, `_shfPalIdx`/`_shfSetPal`); Card-Icons via `_cvPin`/`_cvDumbbell` statt Emoji; Share-Overlay hat branded Hintergrund (Akzent-Glows + Punktraster) statt Schwarz. Segmente: Freundeskarten mit Live-Status („trainiert gerade", `profile.live` aus `isWorkoutActive()`+`timerTs`, Push-Hooks in `startActive`/`finishWk`/`cancelWk`), Online-Dot via RTDB-Presence (`FB.rtdbWatch`), onSnapshot-Live-Updates (`_frStartLive`/`_frStopLive` — Stop beim Tab-Wechsel!), Pull-to-Refresh; Freundesprofil-Sheet (`openFrProfile`: Level, Stats, Badges, Wochen-Vergleich, PRs); Activity-Feed (`profiles/{uid}/activities` Subcollection, Emoji-Reaktionen via reactions-Map, Rules erlauben Fremd-Update NUR auf `reactions`); Freundschaftsanfragen (`requests`-Collection: pending→accepted, Absender löst ein + löscht), Suche (Name-Präfix + Code), QR (`qrcodejs` lazy, Deep-Link `?add=CODE`), Blockieren (`S.blocked`, lokal); Privatsphäre `S.privacy` {gym,live,lastWk,stats,prs,feed} filtert `_pushSocialProfile`-Payload; Gamification: Monats-Ranking (`profile.month`), Challenge 12 Workouts/Monat, Badges. Opt-in via `S.socialOn`; Karte = Leaflet lazy + Carto + Nominatim; Heute-Widget `social`. **Achtung Modul-Zuordnung:** Freundesliste, Rangliste, Karte, Freundschaftsanfragen und `renderFriendsTab`/`setSocTab` liegen in **`js/app-streak.js`**, nicht in `app-community.js` — dort steht der Feed-Pager samt Share-Card/Posts. **Crews** (`js/app-crew.js`, Chip `crew` in `#fr-seg`): feste Gruppe ≤20 mit gemeinsamem Wochenziel, ein Balken für alle, Crew-Streak + Historie (8 Wochen). Collection `crews/{cid}`, **Doc-ID = 6-Zeichen-Beitrittscode** (kein Index, keine Suche, nicht auffindbar); Map `wk:{uid:anzahl}` — jeder schreibt nur `wk.<uid>` und immer **absolut** (kein Doppelzählen, keine Schreibkonflikte). Zählweise `crewCountWeek`: Einheit zählt ab 15 Min. **oder** 6 Sätzen, max. 1 pro Kalendertag, Deckel 7. Wochenschlüssel ist **`crewWeekKeyOf()` (ISO-Montag), NICHT `getWeekKey()`** — dessen Grenze hängt am Wochentag des 1. Januar und verschiebt sich mit der Sommerzeit. Rollover (Streak/Historie) läuft in `FB.runTransaction`, ausgelöst beim Öffnen bzw. `_crewPush()` nach `finishWk`. Beitritt per Code, Deep-Link `gymtrack://crew/CODE` bzw. `?crew=CODE`, QR. Owner ändert Name/Ziel und entfernt Mitglieder; Owner-Austritt löst die Crew auf (keine Übergabe in v1). Lokal `S.crewId` — steht in der `users`-`hasOnly`-Liste **und** in `CLOUD_NEUE_FELDER`. Design-Spec: `docs/superpowers/specs/2026-08-06-crews-design.md`. **Wichtig:** Rules unten (profiles inkl. activities, requests, RTDB-presence-read) müssen in der Firebase-Konsole stehen + neue Keys im users-hasOnly — sonst permission-denied beim KOMPLETTEN users-Push.
...
## Referral (Gratis-Premium) & Universal Links

Wer seinen 7-stelligen Code weitergibt, bekommt bei Einlösung 7 Tage Premium geschenkt — der Geworbene auch. Deckel: 2 Einlösungen JE CODE. Quelle der Wahrheit ist allein der Worker (KV `gymtrack-ref`, `ai-worker/worker.js`), Client-Code in `js/app-referral.js`. Anonyme Konten sind ausgeschlossen. **Ein Konto löst genau einmal im Leben ein — ob frisch installiert oder seit Monaten in Benutzung, spielt keine Rolle.** Design-Spec: `docs/superpowers/specs/2026-08-07-referral-gratiswoche-design.md`.

**Universal Links (seit 07.08.2026):** `https://gymtrack-9q9.pages.dev/?ref=CODE` öffnet auf iOS direkt die installierte App statt Safari. Drei Teile müssen zusammenpassen, sonst fällt der Link still auf den alten Weg (Safari → App Store) zurück:
1. `.well-known/apple-app-site-association` — fängt per `components` **nur** `/?ref=…` ab (privacy.html, Dashboard und die Web-App bleiben im Browser). `build-pages.js` kopiert den Ordner und setzt `Content-Type: application/json`; ohne den verwirft iOS die Datei kommentarlos. Cloudflare Pages liefert `.well-known` als einzigen Punkt-Ordner aus.
2. `ios/App/App/App.entitlements` → `com.apple.developer.associated-domains` = `applinks:<Domain aus GT_WEB>`. **Die App-ID `com.wolter.gymtrack` braucht dafür im Developer-Portal die Capability „Associated Domains"** — fehlt sie, scheitert der Export in `BUILD-FUER-APPSTORE.command` beim Signieren.
3. `_handleRefUrl()` in `js/app-update.js` — gemeinsamer Handler für Universal Link und `gymtrack://ref/CODE`, aufgerufen aus `appUrlOpen` UND `getLaunchUrl`. Die Sperre auf den zuletzt behandelten Code ist Pflicht: der Kaltstart liefert dieselbe Adresse doppelt.

`test/referral.test.js` bewacht App-ID, Domain-Gleichheit, Reichweite des Links und die Code-Erkennung. Wirksam wird das Ganze erst mit einem neuen Store-/TestFlight-Build — eine bereits installierte Version trägt das Entitlement nicht. Empfänger **ohne** App gehen weiter über den App Store und geben den Code von Hand ein (iOS hat keinen Install-Referrer), deshalb steht der Code weiterhin im Teilen-Text.
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    // Crews (js/app-crew.js). Doc-ID = Beitrittscode, deshalb reicht read für
    // jeden Angemeldeten: ohne den geteilten Code findet niemand ein Dokument.
    // Zähl-Zweig hart begrenzt (nur eigener Feldpfad, Wert <= 7); Rollover-Zweig
    // bewusst lose — Rules können 20 Fremdwerte nicht nachrechnen, die
    // Vertrauensgrenze ist die eingeladene Crew. Vollständig in firestore.rules.
    match /crews/{cid} {
      allow read: if request.auth != null;
      // create/update/delete: s. firestore.rules (Beitreten, Verlassen, Zählen,
      // Rollover, Owner-Änderungen, Auflösen) — muss in der Konsole stehen,
      // BEVOR der Client-Code live geht, sonst ist die Collection gesperrt.
    }
